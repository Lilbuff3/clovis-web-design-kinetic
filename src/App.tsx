    ...
    <MotionConfig reducedMotion="user">
      ...
      <main id="main">
        <section id="top" className="hero" aria-labelledby="hero-title" onPointerMove={(event) => {
          ...
          <motion.div className="hero-art" style={{ x: reducedMotion ? 0 : springX, y: reducedMotion ? 0 : springY }} initial={{ scale: reducedMotion ? 1.02 : 1.065, opacity: 0 }} animate={{ scale: 1.02, opacity: 1 }} transition={{ duration: 1.5, ease: [0.22, 1, 0.36, 1] }}>
            <img src="/images/hero-flagship.jpg" alt="Clovis Web Design Central Valley digital craftsmanship and landscape by Adam Youssef." width={1920} height={1080} fetchPriority="high" />
          </motion.div>
      ...
      <AnimatePresence>
        {dialog && <Dialog key="portfolio-dialog" onClose={closeDialog} labelledBy={dialog.type === 'project' ? 'project-title' : dialog.type === 'contact' ? 'contact-title' : 'colophon-title'} className={dialog.type === 'project' ? 'project-dialog' : dialog.type === 'contact' ? 'contact-dialog' : 'colophon-dialog'}>
          ...
          {dialog.type === 'colophon' && <div className="colophon-content"><p className="eyebrow">THE DETAILS BEHIND THE DETAILS</p><Flower className="colophon-flower" /><h2 id="colophon-title">Crafted with<br /><em>integrity.</em></h2><p>This is cloviswebdesign.com, a bespoke web architecture studio founded by Adam Youssef in Clovis, California.</p><dl><div><dt>The visual world</dt><dd>Authentic photography and live components from premier Central Valley clients including Kidney Specialist Inc (Fresno & Madera) and Big Bros Dumpster Rentals (Clovis & Fresno). No fake generic stock imagery.</dd></div><div><dt>The moving parts</dt><dd>Built with React 19, TypeScript, and Motion. Custom in-engine responsive previews and live client components for medical practice triage and contractor flat-rate dispatch.</dd></div><div><dt>The type</dt><dd>Manrope for a clear head. Instrument Serif for classic Central Valley craftsmanship. DM Mono for clinical and engineering precision.</dd></div><div><dt>Your privacy</dt><dd>Zero tracking pixels, zero HIPAA liability, zero platform lock-in. 100% code and asset ownership on day one.</dd></div></dl><button className="text-link" onClick={closeDialog}>Back to the good stuff<ArrowRight size={18} /></button></div>}
        </Dialog>}
      </AnimatePresence>
    </MotionConfig>
  );
